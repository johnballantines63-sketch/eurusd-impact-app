...
### 4. Analyseur Surprise Adapté - SOLUTION HYBRIDE
...
- Problème de connexion DuckDB résolu avec `@st.cache_data`
- Chaque fonction isole sa propre connexion
- `read_only=True` pour lectures, évite conflits
...
- **Analyseur Surprise** adapté (previous comme fallback)
...
- **Erreur DuckDB "No magic bytes"** : Résolu en vidant cache Streamlit
- Si réapparaît : Redémarrer Streamlit (`Ctrl+C` puis relancer)
...
**Après** : `@st.cache_data` + isolation des connexions
**Impact** : Plus de conflit read_only/read_write

### 4. Variables .env non chargées
...
**Système** : ~98% opérationnel avec workarounds
...
**Version sauvegardée** : 06 octobre 2025 - 04:00 UTC  
**État** : Système opérationnel avec solution hybride previous/manual  
**Prochaine priorité** : Tester Analyseur Surprise et saisir premiers forecasts manuels