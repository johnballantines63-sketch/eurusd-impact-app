#!/usr/bin/env python3
"""
SOLUTION ULTIME v2 : Correction erreur connexions multiples
"""
...
DEFAULT_LATENCIES = {
    ...
    'Real_Earnings': {'lat': 8.0, 'ttr': 12.0},
}

def get_default_latency(family_name):
    """Retourne latence/TTR par défaut"""
    return DEFAULT_LATENCIES.get(family_name, {'lat': 10.0, 'ttr': 15.0})
...
def calculate_family_stats(conn, family_name, pattern, db_path):
    """
    Calcule stats en utilisant UNE SEULE connexion DB
    
    Args:
        conn: Connexion DB existante (réutilisée)
    ...
    try:
        ...
        # 3. Compter événements (réutilise connexion existante)
        clean_pattern = pattern.replace('(?i)', '')
...
def main():
    print("=" * 80)
    print("PRÉ-CALCUL ULTIME v2 (connexion unique)")
    print("=" * 80)
    print()
    print("Méthode :")
    print("  - Latence/TTR : Valeurs par défaut observées")
    print("  - MFE : ForecastEngine")
    print("  - N events : Comptage direct")
    print()
    ...
    for idx, (family_name, pattern) in enumerate(FAMILY_PATTERNS.items(), 1):
        ...
        # Calculer stats (passe la connexion existante)
        stats = calculate_family_stats(conn, family_name, pattern, db_path)
    ...
    if success >= 30:
        ...
        print(f"   {success} familles maintenant pré-calculées !")
        print()
        print("   Toutes les familles importantes = ULTRA-RAPIDES ⚡")
    elif success > 5:
        print(f"🎉 AMÉLIORATION MAJEURE !")
        print(f"   Avant: 5 → Maintenant: {success} familles")
