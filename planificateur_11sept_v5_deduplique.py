"""
PLANIFICATEUR 11 SEPTEMBRE 2025 - V5 DÉDUPLIQUÉ

SOLUTION FINALE:
1. Charger events + event_families (scores BRUTS de la famille)
2. Filtrer sur 12:30 UTC (14:30 heure locale Berne)
3. DÉDUPLICATION : Un seul événement par (family, ts_utc)
4. Calculer surprise pour CHAQUE événement individuel
5. Ajuster score selon surprise INDIVIDUELLE avec calculate_adjusted_empirical_score()
6. Appliquer Formule D complète
...
EVENT_TIME_UTC = "12:30:00"  # Heure UTC (14:30 Berne)
REFERENCE_MT5 = {
    'total_pips': 56.2,
    'phase1_pips': 37.4,
    'pullback_pips': 27.1
...
def load_events_deduplicated():
    """
    Charge événements depuis events + event_families.
    DÉDUPLICATION : Un seul événement par (family, ts_utc) via DISTINCT ON.
    """
    db_path = get_db_path()
    conn = duckdb.connect(str(db_path))
    
    # Query avec DISTINCT ON pour déduplication
    # Garde le premier événement par (family, ts_utc) trié par event_key
    query = f"""
    WITH ranked_events AS (
        SELECT 
            ...
            ROW_NUMBER() OVER (PARTITION BY ef.family, e.ts_utc ORDER BY e.event_key) as rn
        FROM events e
        ...
        WHERE DATE(e.ts_utc) = '{EVENT_DATE}'
            ...
            AND CAST(e.ts_utc AS TIME) = '{EVENT_TIME_UTC}'
    )
    SELECT 
        ts_utc,
        event_key,
    ...
    if not df.empty:
        ...
        # Calculer surprise_pct (gérer NaN et division par 0)
        df['surprise_pct'] = df.apply(
            lambda row: (row['surprise'] / row['forecast'] * 100) 
                if pd.notna(row['surprise']) and pd.notna(row['forecast']) and row['forecast'] != 0 
                else 0,
            axis=1
        )
...
def calculate_phases(events_df):
    ...
    """
    print("\n" + "="*80)
    print("📊 CALCUL DES PHASES - FORMULE D (dédupliqué)")
    print("="*80)
    
    print(f"\n✅ {len(events_df)} événements DÉDUPLIQUÉS à {EVENT_TIME_UTC} UTC")
    ...
    for _, event in events_df.iterrows():
        family = event['family']
        score_adjusted = event['adjusted_score']
...
def main():
    print("\n" + "="*80)
    print("🧪 PLANIFICATEUR 11 SEPTEMBRE 2025 - V5 DÉDUPLIQUÉ")
    print("="*80)
    print("\n✅ Filtre 12:30 UTC + Déduplication par famille")
    print("   events + event_families (scores bruts + ajustement individuel)")
    
    # Charger événements
    print("\n📂 CHARGEMENT DONNÉES...")
    events_df = load_events_deduplicated()
    
    if events_df.empty:
        print("❌ Aucun événement trouvé")
        return
    
    print(f"✅ {len(events_df)} événements chargés (dédupliqués)")
    ...
    if results['mae'] < 10:
        print(f"\n   🎉 VALIDATION RÉUSSIE !")
        print(f"   ✅ Bug double ajustement RÉSOLU !")
        print(f"\n   💡 Solution : events + event_families avec déduplication")
    else:
        print(f"\n   ⚠️  À ANALYSER...")
