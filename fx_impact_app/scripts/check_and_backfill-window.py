# fx_impact_app/scripts/check_and_backfill_window.py
...
def _parse_t_series_to_datetime_utc(s: pd.Series) -> pd.Series:
    """Décode s (champ 't') en UTC en autodétectant l’unité."""
    x = pd.to_numeric(s, errors="coerce")
    mx = x.dropna().max()
    if pd.isna(mx):
        return pd.to_datetime(s, errors="coerce", utc=True)  # tente ISO
    # Heuristique d’unité
    # ~1.7e9 = secondes (2025)
    # ~1.7e12 = millisecondes
    # ~1.7e15 = microsecondes
    # ~1.7e18 = nanosecondes
    if mx > 1e17:
        unit = "ns"
    elif mx > 1e14:
...
def fetch_intraday_eodhd(symbol: str,
    ...
    # Détection colonnes
    dt_col = next((c for c in df.columns if str(c).lower() in ("datetime","timestamp","date","time","t")), None)
    px_col = next((c for c in df.columns if str(c).lower() in ("close","c","price","last")), None)
    ...
    if str(dt_col).lower() == "t":
        dt = _parse_t_series_to_datetime_utc(df[dt_col])
    else:
        dt = pd.to_datetime(df[dt_col], errors="coerce", utc=True)
        # Si le champ avait un TZ local, to_datetime(utc=True) le convertit déjà
    ...
    # élimine dates aberrantes (1970 & co)
    out = out[out["datetime"] >= pd.Timestamp("2000-01-01", tz="UTC")]
    # dédoublonnage
    out = out.sort_values("datetime", kind="stable").drop_duplicates(subset=["datetime"], keep="last").reset_index(drop=True)
    return out
...
def backfill_chunks(symbol: str,
    ...
    if parts:
        ...
        # filet de sécu final
        out = out[out["datetime"].between(start_utc, end_utc)]
        out = out[out["datetime"] >= pd.Timestamp("2000-01-01", tz="UTC")]
        out = out.sort_values("datetime", kind="stable").drop_duplicates(subset=["datetime"], keep="last")
        return out.reset_index(drop=True), logs
...
def main():
    ...
    with duckdb.connect(db_path) as con:
        ...
        if fetched > 0:
            # Dédoublonnage/filtrage final (sécurité)
            df = df[df["datetime"].between(start_utc, end_utc)]
            df = df[df["datetime"] >= pd.Timestamp("2000-01-01", tz="UTC")]
