#!/usr/bin/env python3
"""
SESSION 73 - PHASE 2 : CROISER MOUVEMENTS AVEC EVENTS (Version Timezone Corrigée)

CORRECTION CRITIQUE : Events stockés en UTC+2 (Berne), prices en UTC
Solution : Ajouter 2h au movement_time avant de chercher events
...
- Pour chaque mouvement : query events dans ±10 min (après correction timezone)
...
TIMEZONE_OFFSET_HOURS = 2  # Events en UTC+2 (Berne), prices en UTC
...
def get_events_for_movement(conn, movement_time, window_minutes=10, tz_offset_hours=2):
    """
    Récupérer événements dans fenêtre ±window_minutes autour du mouvement.
    
    CORRECTION TIMEZONE : Ajoute tz_offset_hours au movement_time car events 
    sont stockés en UTC+2 (Berne) alors que prices sont en UTC.
    
    Args:
        conn: Connexion DuckDB
        movement_time: Datetime du mouvement UTC (str ou datetime)
        window_minutes: Fenêtre temporelle ±N minutes
        tz_offset_hours: Offset timezone (2 pour Berne)
    ...
    # CORRECTION : Ajouter offset timezone
    # Si movement_time = 12:30 UTC → chercher events autour de 14:30 UTC+2
    query = f"""
    SELECT 
        e.ts_utc,
        e.event_key,
    ...
    WHERE e.ts_utc >= ('{movement_time}'::TIMESTAMP + INTERVAL '{tz_offset_hours} hours') - INTERVAL '{window_minutes} minutes'
      AND e.ts_utc <= ('{movement_time}'::TIMESTAMP + INTERVAL '{tz_offset_hours} hours') + INTERVAL '{window_minutes} minutes'
    ORDER BY e.ts_utc
    """
...
def main():
    print("\n" + "="*80)
    print("SESSION 73 - PHASE 2 : CROISER MOUVEMENTS AVEC EVENTS (Timezone Corrigée)")
    print("="*80)
    ...
    print(f"   ⚠️  CORRECTION TIMEZONE : Events UTC+{TIMEZONE_OFFSET_HOURS} (Berne), Prices UTC")
    ...
    for idx, movement in df_movements.iterrows():
        ...
        # Chercher événements (avec correction timezone)
        events_df = get_events_for_movement(
            conn, 
            movement_time, 
            TIME_WINDOW_MINUTES,
    ...
    if len(df_with_events) > 0:
        ...
        print(f"\n   🏆 Top 5 mouvements AVEC événements :")
        for _, row in df_with_events.nlargest(5, 'impact_reel_pips').iterrows():
            print(f"      {row['date']} {row['time']} : {row['impact_reel_pips']:.1f} pips ({row['nb_events']:.0f} events)")
    ...
    print(f"\n✅ PHASE 2 TERMINÉE (Timezone Corrigée)")
    print("="*80)
