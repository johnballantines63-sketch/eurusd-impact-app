"""
SCRIPT 1 V3 - SCANNER MOUVEMENTS FILTRÉ (SESSION 75)
====================================================

CORRECTION FINALE : Retirer filtre importance (colonne cassée)
Utiliser : Event title + Score DB > 0
...
OUTPUT_PATH = SCRIPT_DIR / "dataset_session75_filtered.csv"

# Critères filtrage
COUNTRIES = ['US', 'EU']  # Pays majeurs
SURPRISE_MAX = 100.0  # % - Surprises réalistes
NB_EVENTS_MIN = 3  # Vrais clusters
IMPACT_MIN = 40  # pips - Mouvements significatifs
SCORE_MIN = 10.0  # Score DB > 0 (events connus, pas Unknown)
...
def cross_with_events_filtered(conn, movements):
    """
    ✅ CORRECTION FINALE : Retirer filtre importance
    Filtrer par : Pays + Score > 10 + Surprise < 100% + Nb events ≥ 3
    """
    print(f"\n{'='*70}")
    ...
    print(f"   Score DB : > {SCORE_MIN} (events connus)")
    ...
    for i, mov in enumerate(movements):
        ...
        # ✅ Filtre score : Events connus uniquement
        ...
        has_high_importance = True  # Assumé (score > 10)
...
def main():
    print(f"\n{'='*70}")
    print("SCANNER V2 FILTRÉ - SESSION 75 (SANS FILTRE IMPORTANCE)")
    print(f"{'='*70}")
