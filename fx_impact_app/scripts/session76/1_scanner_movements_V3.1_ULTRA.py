"""
SCRIPT 1 V3.1 - SCANNER ULTRA-ASSOUPLÉ (SESSION 76 - BACKUP)
=============================================================

UTILISER SI V3 STANDARD < 30 MOUVEMENTS

CRITÈRES ULTRA-ASSOUPLIS :
- Score > 3 (vs 5) - Tous events DB
- Surprise < 300% (vs 200%) - Très tolérant
- Nb events ≥ 2 (identique) - Minimum clusters
- Impact ≥ 25 pips (vs 30) - Mouvements petits
- Top 75 par année (vs 50) - Maximum mouvements
- Années : 2023-2025 (identique)
...
# ════════════════════════════════════════════════════════════════
# CONFIGURATION ULTRA-ASSOUPLIE
# ════════════════════════════════════════════════════════════════

SCRIPT_DIR = Path(__file__).parent
DB_PATH = fx_app_path / "data" / "warehouse.duckdb"
OUTPUT_PATH = SCRIPT_DIR / "dataset_session76_ultra.csv"

# ✅ CRITÈRES ULTRA-ASSOUPLIS
COUNTRIES = ['US', 'EU']
SCORE_MIN = 3.0  # vs 5.0 - Tous events DB
SURPRISE_MAX = 300.0  # vs 200.0 - Très tolérant
NB_EVENTS_MIN = 2  # identique
IMPACT_MIN = 25  # vs 30 pips - Mouvements petits
TOP_N_PER_YEAR = 75  # vs 50 - Maximum mouvements
...
def cross_with_events_ultra(conn, movements):
    """
    Croisement avec events + filtres ULTRA-ASSOUPLIS
    
    Filtres :
    - Score DB > 3 (tous events DB)
    - Surprise < 300% (très tolérant)
    - Nb events ≥ 2 (minimum clusters)
    - Impact ≥ 25 pips (mouvements petits)
    """
    print(f"\n{'='*70}")
    print("CROISEMENT AVEC EVENTS + FILTRES ULTRA-ASSOUPLIS")
    print(f"{'='*70}")
    
    print(f"Filtres appliqués (ULTRA-ASSOUPLIS) :")
    print(f"   Pays : {', '.join(COUNTRIES)}")
    print(f"   Score DB : > {SCORE_MIN} (tous events DB)")
    print(f"   Surprise : < {SURPRISE_MAX}% (très tolérant)")
    print(f"   Nb events : ≥ {NB_EVENTS_MIN} (minimum)")
    print(f"   Impact : ≥ {IMPACT_MIN} pips (petits mouvements)")
    ...
    for i, mov in enumerate(movements):
        ...
        has_high_importance = (score_moyen >= 10.0)
...
def main():
    ...
    print(f"\n{'='*70}")
    print("SCANNER V3.1 ULTRA-ASSOUPLÉ - SESSION 76 (BACKUP)")
    print(f"{'='*70}")
    print(f"Date : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    
    print("⚠️  À UTILISER SI V3 STANDARD < 30 MOUVEMENTS")
    print()
    print("CRITÈRES ULTRA-ASSOUPLIS :")
    print(f"   Score > {SCORE_MIN} (tous events DB)")
    print(f"   Surprise < {SURPRISE_MAX}% (très tolérant)")
    print(f"   Nb events ≥ {NB_EVENTS_MIN} (minimum)")
    print(f"   Impact ≥ {IMPACT_MIN} pips (petits mouvements)")
    print(f"   Top {TOP_N_PER_YEAR} par année (maximum)")
    ...
    try:
        ...
        if len(all_movements) == 0:
            print(f"⚠️  AUCUN MOUVEMENT DÉTECTÉ")
            return 0
        ...
        for year in YEARS:
            ...
            if len(df_year) > 0:
                ...
                print(f"Année {year} : 0 mouvements")
        
        print(f"✅ Total : {len(movements_top)} mouvements")
        
        movements_filtered = cross_with_events_ultra(conn, movements_top)
        ...
        if movements_filtered:
            ...
            print(f"\n✅ SCANNER V3.1 ULTRA TERMINÉ")
            print(f"\n📊 RÉSUMÉ FINAL :")
            print(f"   Dataset ultra : {len(df_final)} mouvements")
            ...
            if len(df_final) >= 30:
                ...
                print(f"   Considérer Option C : Ajouter pays UK/JP/CH")
