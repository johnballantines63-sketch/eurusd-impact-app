#!/usr/bin/env python3
"""
Créer Dataset Complet - Session 73 - VERSION CORRIGÉE
Croiser mouvements forts avec événements DB

CORRECTION : Gestion timezone UTC+2 → UTC
...
def get_events_for_movement(
    ...
    CORRECTION : Convertir movement_datetime (UTC+2) en UTC avant recherche
    
    Args:
        conn: Connexion DuckDB
        movement_datetime: Datetime du mouvement (peak) en UTC+2
    ...
    # CORRECTION : Convertir en UTC si timezone aware
    if movement_datetime.tzinfo is not None:
        movement_datetime_utc = movement_datetime.astimezone(datetime.now().astimezone().tzinfo).replace(tzinfo=None)
        # Convertir en UTC pur
        movement_datetime_utc = pd.Timestamp(movement_datetime).tz_convert('UTC').tz_localize(None)
    else:
        movement_datetime_utc = movement_datetime
    
    # Fenêtre temporelle en UTC
    start_time = movement_datetime_utc - timedelta(minutes=time_window_minutes)
    end_time = movement_datetime_utc + timedelta(minutes=time_window_minutes)
...
def calculate_cluster_metrics(df_events: pd.DataFrame, movement_direction: str) -> dict:
    ...
    # Gérer les None dans event_title
    event_titles = [str(title) if title is not None else 'Unknown' for title in df_events['event_title'].tolist()]
    events_list = ' | '.join(event_titles)
...
def create_complete_dataset(
    ...
    CORRECTION : Fenêtre élargie à ±30 min (au lieu de ±10 min)
    ...
    print(f"\n{'='*70}")
    print(f"CRÉATION DATASET COMPLET - SESSION 73 - VERSION CORRIGÉE")
    print(f"{'='*70}\n")
    ...
    print(f"   ⚠️  CORRECTION TIMEZONE : Conversion UTC+2 → UTC appliquée")
...
def analyze_dataset_statistics(df: pd.DataFrame):
    ...
    with_events = (df['nb_events'] > 0).sum()
    print(f"\n📊 COUVERTURE ÉVÉNEMENTS :")
    print(f"   Mouvements AVEC événements : {with_events} ({with_events/len(df)*100:.1f}%)")
    print(f"   Mouvements SANS événements : {no_events} ({no_events/len(df)*100:.1f}%)")
    
    if no_events > 0:
        print(f"\n   → Mouvements sans événements probablement dus à :")
...
def main():
    """
    Main : Créer dataset complet - VERSION CORRIGÉE
    """
    
    print("\n" + "="*70)
    print("SESSION 73 - CRÉATION DATASET COMPLET - VERSION CORRIGÉE")
    print("Correction timezone UTC+2 → UTC")
    print("="*70)
    
    # Paramètres
    movements_csv = MOVEMENTS_CSV
    db_path = DB_PATH
    output_csv = fx_impact_app_path / "data" / "dataset_complete_session73_FIXED.csv"
    ...
    df_dataset = create_complete_dataset(
        ...
        db_path=str(db_path),
        time_window_minutes=30  # CORRECTION : Fenêtre élargie
    )
    ...
    print(f"\n{'='*70}")
    print(f"✅ DATASET CRÉÉ AVEC SUCCÈS - VERSION CORRIGÉE")
    print(f"{'='*70}\n")
    
    print(f"🎯 Prochaine étape :")
    print(f"   Exécuter analyze_correlations_session73.py avec dataset_complete_session73_FIXED.csv")
