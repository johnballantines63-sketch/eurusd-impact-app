...
try:
    from sequence_multi_event_timeline_v87 import sequence_multi_event_timeline, calculate_ttr_accuracy_stats  # ✅ SESSION 47
...
def load_precomputed_stats_from_db():
    ...
    try:
        ...
        stats_dict = {}
        for row in results:
            family_db = row[0]  # Nom tel que stocké en DB (peut avoir underscore)
            stats = {
...
def predict_impact_fast(family, surprise, precomputed_stats, years_back=3):
    ...
    # 🔧 CORRECTION SESSION 41 : Ne PAS normaliser, utiliser le nom exact
    # La DB stocke les noms avec espaces comme dans event_families.py
    if family in precomputed_stats:
        ...
        if ttr_corrected > 20:
            ...
            print(f"🔧 TTR corrigé pour {family}: "
                    ...
                    try:
                        with st.spinner("🔄 Calcul timeline séquentielle..."):
                            ...
                            phases = sequence_multi_event_timeline(
                                ...
                                # ❌ SESSION 47 : Paramètre debug n'existe pas dans la fonction
                            )
