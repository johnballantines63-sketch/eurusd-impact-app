...
def get_cpi_events_for_date(target_date: datetime) -> pd.DataFrame:
    ...
    MÉTHODE EXACTE de test_planificateur_v2_final.py
    
    Args:
        target_date: Date cible
    
    Returns:
        DataFrame des événements CPI
    """
    conn = get_db_connection()
    
    date_str = target_date.strftime('%Y-%m-%d')
    
    # Query EXACTE de test_planificateur_v2_final.py
    query = """
    SELECT 
        e.event_key,
        e.label,
    ...
    # Filtrer CPI UNIQUEMENT (ligne 57-58 de test_planificateur_v2_final.py)
...
if st.button("🎯 Calculer Prédictions", type="primary"):
    ...
    if cpi_events.empty:
        ...
        st.info("💡 La méthode Session 55 ne traite que les événements CPI (Consumer Price Index)")
        st.stop()
    
    st.success(f"✅ {len(cpi_events)} événement(s) CPI trouvé(s)")
    
    with st.spinner("Calcul avec formules validées Session 55..."):
        predictions = calculate_predictions(cpi_events)
