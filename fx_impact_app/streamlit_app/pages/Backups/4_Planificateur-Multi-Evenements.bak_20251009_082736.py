...
def get_event_direction(family, surprise):
    ...
    Logique :
    - Pour US events (USD impact):
      * Good US news → USD UP → EUR/USD DOWN (direction = -1)
      * Bad US news → USD DOWN → EUR/USD UP (direction = 1)
    
    - Familles INVERSÉES (Jobless, Unemployment, Inflation, CPI):
      * Surprise positive → BAD news → direction = UP (+1) car USD faiblit
      
    - Familles NORMALES (GDP, NFP, Retail, etc):
      * Surprise positive → GOOD news → direction = DOWN (-1) car USD se renforce
    """
    family_normalized = family.replace(' ', '_')
    sentiment = FAMILY_SENTIMENT.get(family_normalized, 1)  # Default: normal
    
    # Pour US data : Good news for USD = Bad for EUR/USD
    # Si sentiment = -1 (inversé) : surprise+ = bad news = USD down = EUR/USD up
    # Si sentiment = 1 (normal) : surprise+ = good news = USD up = EUR/USD down
    
    if surprise > 0:
        # Surprise positive
        direction = sentiment  # Si inversé: +1 (UP), si normal: +1 mais on inverse après
    else:
        # Surprise négative
        direction = -sentiment
    
    # Pour les familles normales, inverser car good USD news = bad EUR/USD
    if sentiment == 1:
        direction = -direction
                    ...
                    if show_charts:
                        for chart_idx, result in enumerate(backtest_results):
                            ...
                            with st.expander(f"📊 {pred['event']['family']} - {result['event_time'].strftime('%d/%m/%Y %H:%M')}", expanded=False):
                                ...
                                # ⚠️ Note : En mode multi-événements, le mouvement de prix
                                # est influencé par TOUS les événements du jour.
                                # Ce graphique montre la prédiction individuelle vs mouvement total.
                                ...
                                st.plotly_chart(chart, use_container_width=True, key=f"backtest_chart_{chart_idx}")
