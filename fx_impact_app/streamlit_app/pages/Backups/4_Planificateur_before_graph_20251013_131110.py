                ...
                if use_sequential:
                    ...
                    try:
                        with st.spinner("🔄 Calcul timeline séquentielle..."):
                            ...
                            # ═══════════════════════════════════════════════════════════
                            # 📈 NOUVELLE SECTION : GRAPHIQUE PRÉDICTION MINUTE PAR MINUTE
                            # ═══════════════════════════════════════════════════════════
                            
                            st.subheader("📈 Évolution Prédite du Cours EUR/USD")
                            st.markdown("**Visualisez l'évolution minute par minute du cours selon les prédictions**")
                            
                            # Input prix de départ
                            col_price_input, col_price_info = st.columns([2, 3])
                            
                            with col_price_input:
                                start_price_input = st.number_input(
                                    "💵 Prix EUR/USD actuel (ou juste avant événement)",
                                    min_value=0.9000,
                                    max_value=1.2000,
                                    value=1.0950,
                                    step=0.0001,
                                    format="%.4f",
                                    key="start_price_input",
                                    help="Entrez le cours EUR/USD actuel pour simuler l'évolution prédite"
                                )
                            
                            with col_price_info:
                                st.info(
                                    f"💡 **Comment l'utiliser ?**\n\n"
                                    f"1. Entrez le cours EUR/USD juste avant les événements\n"
                                    f"2. Le graphique montrera l'évolution minute par minute prédite\n"
                                    f"3. Impact total prédit : {abs(vectorial_impact):.1f} pips {combined_direction}"
                                )
                            
                            # Paramètres avancés
                            with st.expander("⚙️ Paramètres Avancés", expanded=False):
                                col_dur, col_vol, col_spread = st.columns(3)
                                
                                with col_dur:
                                    duration_minutes = st.slider(
                                        "Durée de simulation (min)",
                                        min_value=30,
                                        max_value=180,
                                        value=90,
                                        step=15,
                                        help="Durée totale du graphique"
                                    )
                                ...
                                with col_vol:
                                    volatility_factor = st.slider(
                                        ...
                                        value=0.3,
                                        step=0.1,
                                        help="0 = lisse, 1 = très volatile"
                                    )
                                
                                with col_spread:
                                    spread_pips = st.selectbox(
                                        "Spread (pips)",
                                        options=[0.0, 0.5, 1.0, 1.5, 2.0, 5.0, 10.0, 20.0],
                                        index=3,  # Défaut 1.5 pips
                                        help="Spread bid/ask à afficher sur le graphique"
                                    )
                                
                                show_spread = st.checkbox(
                                    "Afficher lignes Bid/Ask",
                                    value=True if spread_pips > 0 else False,
                                    help="Afficher les lignes bid et ask en plus du mid-price"
                                )
                            
                            # Générer courbe
                            if st.button("📊 Générer Graphique de Prédiction", type="primary", use_container_width=True):
                                with st.spinner("🔄 Génération de la courbe prédictive..."):
                                    try:
                                        # Préparer données pour génération
                                        predictions_for_curve = []
                                        for pred in predictions:
                                            predictions_for_curve.append({
                                                'event_time': pd.to_datetime(pred['event']['ts_utc']),
                                                'predicted_pips': pred['predicted_pips'],
                                                'direction': pred['direction'],
                                                'latency_median': pred['latency_median'],
                                        ...
                                        # Générer courbe multi-événements avec chandeliers
                                        base_time = min(p['event_time'] for p in predictions_for_curve)
                                        
                                        price_df = generate_candlestick_curve_multi_events(
                                            start_price=start_price_input,
                                            predictions=predictions_for_curve,
                                            base_time=base_time,
                                        ...
                                        fib_levels_prices = calculate_fibonacci_price_levels(
                                            start_price=start_price_input,
                                            impact_pips=abs(vectorial_impact),
                                            direction=1 if vectorial_impact > 0 else -1
                                        )
                                        
                                        # Créer marqueurs d'événements
                                        event_markers = []
                                        for idx, pred in enumerate(predictions):
                                            event_time = pd.to_datetime(pred['event']['ts_utc'])
                                            event_markers.append({
                                                'time': event_time,
                                                'label': f"📊 {pred['event']['family'][:15]}...",
                                                'color': 'black' if idx == 0 else 'gray'
                                            })
                                        
                                        # Créer graphique chandeliers
                                        fig = create_candlestick_prediction_chart(
                                            price_df=price_df,
                                            start_price=start_price_input,
                                            total_impact_pips=abs(vectorial_impact),
                                            direction=1 if vectorial_impact > 0 else -1,
                                            event_markers=event_markers,
                                            fib_levels=fib_levels_prices,
                                            show_spread=show_spread,
                                            title=f"Évolution Prédite EUR/USD (Chandeliers) - {base_time.strftime('%d/%m/%Y %H:%M')}"
                                        )
                                        
                                        # Ajouter statistiques
                                        fig = add_statistics_to_chart(
                                            fig=fig,
                                            price_df=price_df,
                                            start_price=start_price_input,
                                            predicted_impact=vectorial_impact,
                                            direction=1 if vectorial_impact > 0 else -1
                                        )
                                        
                                        # Afficher graphique
                                        st.plotly_chart(fig, use_container_width=True, key="price_prediction_chart")
                                        
                                        # Métriques de la courbe
                                        ...
                                        with col_m3:
                                            st.metric(
                                                "Prix Final Close (T+{} min)".format(duration_minutes),
                                                f"{end_price:.5f}",
                                        ...
                                        with col_m4:
                                            ...
                                            st.metric(
                                                "Amplitude Totale",
                                                f"{amplitude:.1f} pips",
                                                help="Différence entre high max et low min"
                                            )
                                        
                                        # Afficher spread si activé
                                        if spread_pips > 0:
                                            st.info(f"📊 **Spread appliqué** : {spread_pips} pips (Bid-Ask)")
                                        
                                        st.success("✅ Graphique de prédiction généré avec succès !")
                                        
                                        # Recommandations de trading
                                        st.markdown("""\n                                        **📋 Comment utiliser ce graphique :**\n                                        \n                                        1. **Zones Fibonacci** : Lignes en pointillés montrant les niveaux de retracement\n                                        2. **Prix de départ** : Ligne grise horizontale\n                                        3. **Événements** : Lignes verticales noires/grises\n                                        4. **Zones colorées** : \n                                           - Gris = Latence (prix stable)\n                                           - Vert = Mouvement (progression)\n                                           - Rouge = Retracement (retour partiel)\n                                        \n                                        💡 **Note** : Ce graphique est une *simulation* basée sur les statistiques historiques.\n                                        Les mouvements réels peuvent varier selon le contexte de marché.\n                                        """)
