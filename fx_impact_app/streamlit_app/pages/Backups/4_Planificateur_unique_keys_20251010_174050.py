    ...
    **Méthode vectorielle** :
    ...
    with col_btn1:
        if st.button("✅ Tout sélectionner", use_container_width=True):
            # Sélectionner par event_key au lieu de idx
            for idx, event in df.iterrows():
                event_id = f"{event['event_key']}_{event['ts_utc']}".replace(' ', '_').replace(':', '').replace('+', '').replace('-', '_')
                st.session_state.selection_state[event_id] = True
            st.rerun()

    with col_btn2:
        if st.button("❌ Tout désélectionner", use_container_width=True):
            # Désélectionner par event_key au lieu de idx
            for idx, event in df.iterrows():
                event_id = f"{event['event_key']}_{event['ts_utc']}".replace(' ', '_').replace(':', '').replace('+', '').replace('-', '_')
                st.session_state.selection_state[event_id] = False
            st.rerun()
    ...
    for date in dates:
        ...
        for idx, event in day_events.iterrows():
            ...
            with col1:
                # Créer ID unique pour cet événement
                event_id = f"{event['event_key']}_{event['ts_utc']}".replace(' ', '_').replace(':', '').replace('+', '').replace('-', '_')
                
                # Initialiser état si absent
                if event_id not in st.session_state.selection_state:
                    st.session_state.selection_state[event_id] = True
                
                # Lire depuis selection_state
                checked = st.checkbox(
                    "",
                    value=st.session_state.selection_state[event_id],
                    key=f"check_{event_id}"
                )
                
                # Mettre à jour selection_state
                # st.session_state.selection_state[event_id] = checked  # ❌ DÉSACTIVÉ: conflit avec boutons
