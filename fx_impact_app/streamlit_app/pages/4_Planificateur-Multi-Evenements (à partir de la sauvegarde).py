...
Version 8.6 : Pullback entre phases rapprochées + Facteur d'atténuation adaptatif
"""
...
# === NOUVEAU : Timeline séquentielle v8.6 ===
import sys
from pathlib import Path
...
try:
    from sequence_multi_event_timeline_v86 import sequence_multi_event_timeline, calculate_ttr_accuracy_stats
    from streamlit_sequential_ui import display_sequential_timeline
    SEQUENTIAL_MODE_AVAILABLE = True
    print("🚀 [4_Planificateur] Module v8.6 (avec pullback) importé avec succès !")
...
# ═══════════════════════════════════════════════════════════════
# 🔄 FONCTION REFRESH DONNÉES DU JOUR (EODHD)
# ═══════════════════════════════════════════════════════════════

def refresh_today_events():
    """Mise à jour événements du jour depuis EODHD API"""
    ...
    try:
        ...
        if not items:
            st.warning("⚠️ Aucun événement retourné par EODHD")
            return False
        ...
        if df.empty:
            st.warning("⚠️ Aucun événement valide")
            return False
...
st.markdown("**Prédictions combinées pour événements simultanés avec latence, TTR et retracement**")
st.caption("🔖 Version 8.6 : TTR réel calculé depuis les prix observés + Pullback entre phases rapprochées + Facteur d'atténuation adaptatif")
...
def get_future_events(date_from, date_to, countries):
    ...
    if len(df) > 0:
        ...
        # APRÈS: Tous événements chargés, même sans famille
...
if st.sidebar.button("🔍 Charger Événements", type="primary", use_container_width=True):
    with st.spinner("Chargement des événements..."):
        ...
        st.sidebar.success(f"✅ {total_count} événements chargés")
        st.sidebar.info(f"📊 {mapped_count} avec famille | {unmapped_count} sans famille")

# 🔄 Bouton rafraîchir données
st.sidebar.divider()
st.sidebar.markdown("### 🔄 Données du jour")

if st.sidebar.button("📥 Rafraîchir depuis EODHD", use_container_width=True, 
                     help="Met à jour les événements du jour depuis EODHD"):
...
st.sidebar.caption("💡 Ou lance: python3 update_today_events.py")
...
if not st.session_state.events_loaded:
    ...
    **Méthode vectorielle** :
    ...
    # 🆕 SOLUTION SIMPLE : Boutons avec logique claire
    
    # État global : True = tout sélectionné
    ...
    with col_btn1:
        if st.button("✅ Tout sélectionner", use_container_width=True):
            # Modifier DIRECTEMENT l'état de chaque checkbox
            for date in dates:
    ...
    # Initialiser selection_state si besoin
    if 'selection_state' not in st.session_state:
        st.session_state.selection_state = {}
    ...
    for date in dates:
        ...
        for idx, event in day_events.iterrows():
            # Ignorer les événements sans famille
    ...
    if len(st.session_state.selected_events) > 0:
        ...
        for idx in sorted(st.session_state.selected_events):
            ...
            with st.expander(f"📊 {event['family']} - {pd.to_datetime(event['ts_utc']).strftime('%H:%M')} ({event['country']})", expanded=True):
                ...
                # Prédiction individuelle (seulement si famille définie)
                ...
                if use_sequential:
                    ...
                    try:
                        with st.spinner("🔄 Calcul timeline séquentielle..."):
                            ...
                            st.subheader("📈 Évolution Prédite du Cours EUR/USD (Minute par Minute)")
                            
                            st.markdown("""
                            Visualisez l'évolution minute par minute prédite du cours EUR/USD basée sur les 
                            événements sélectionnés. Le graphique affiche des **chandeliers OHLC** avec spread 
                            Bid/Ask et niveaux de retracement Fibonacci.
                            """)
                            
                            # Paramètres du graphique
                            with st.expander("⚙️ Paramètres du Graphique", expanded=True):
                                col_g1, col_g2, col_g3, col_g4 = st.columns(4)
                                
                                with col_g1:
                                    start_price_input = st.number_input(
                                        "💵 Prix EUR/USD actuel",
                                        min_value=1.0000,
                                        max_value=1.2000,
                                        value=1.0950,
                                        step=0.0001,
                                        format="%.4f",
                                        help="Prix de départ pour la simulation",
                                        key="minute_chart_price_155755_6050"
                                    )
                                ...
                                with col_g2:
                                    spread_pips = st.number_input(
                                        ...
                                        value=1.0,
                                        step=0.1,
                                        help="Spread Bid/Ask en pips",
                                        key="minute_chart_spread_155755_6050"
                                    )
                                
                                with col_g3:
                                    duration_minutes = st.number_input(
                                        "⏱️ Durée simulation (min)",
                                        min_value=30,
                                        max_value=240,
                                        value=120,
                                        step=10,
                                        help="Durée totale de la simulation",
                                        key="minute_chart_duration_155755_6050"
                                    )
                                ...
                                with col_g4:
                                    volatility_factor = st.slider(
                                        ...
                                        value=0.3,
                                        step=0.1,
                                        help="Facteur de volatilité du marché (0.1=calme, 1.0=volatil)",
                                        key="minute_chart_volatility_155755_6050"
                                    )
                                
                                # Options avancées
                                with st.expander("⚙️ Options Avancées"):
                                    show_bid_ask = st.checkbox(
                                        "Afficher lignes Bid/Ask",
                                        value=True,
                                        help="Affiche les lignes de prix Bid et Ask",
                                        key="minute_chart_bid_ask_155755_6050"
                                    )
                                    
                                    show_fibonacci = st.checkbox(
                                        "Afficher niveaux Fibonacci",
                                        value=True,
                                        help="Affiche les niveaux de retracement Fibonacci",
                                        key="minute_chart_fibonacci_155755_6050"
                                    )
                            
                            # Bouton génération
                            if st.button("🎨 Générer Graphique de Prédiction", type="primary", use_container_width=True, key="minute_chart_generate_155755_6050"):
                                with st.spinner("🎨 Génération du graphique minute par minute..."):
                                    try:
                                        # Préparer données événements
                                        # ═══════════════════════════════════════════════════════════
                                        # ✅ CORRECTION CRITIQUE : Utiliser PHASES au lieu de PREDICTIONS
                                        # ═══════════════════════════════════════════════════════════
                                        
                                        # Vérifier si phases existent (calculées dans mode séquentiel)
                                        if 'phases' in locals() and phases and len(phases) > 0:
                                            # ✅ UTILISER LES PHASES CALCULÉES !
                                            events_for_generator = []
                                            
                                            for phase in phases:
                                                events_for_generator.append({
                                                    'event_time': pd.to_datetime(phase['start_time']),
                                                    'predicted_pips': phase['impact_combined'],  # ← Impact VECTORIEL COMBINÉ
                                                    'direction': 1 if phase['direction'] == 'UP' else -1,
                                                    'latency_median': phase['latency_minutes'],
                                                    'ttr_median': phase.get('ttr_real', phase['ttr_predicted']),  # ← TTR OBSERVÉ !
                                                    'label': f"Phase {phase['phase_num']} ({phase['num_events']} event(s))",
                                                    'is_phase': True,
                                                    'phase_num': phase['phase_num']
                                                })
                                            
                                            st.success(f"✅ Graphique basé sur {len(phases)} phases calculées avec TTR observés")
                                            
                                        else:
                                            # ⚠️ FALLBACK : utiliser predictions si pas de phases
                                            # ✅ CORRECTION V2 : Utiliser UN événement avec impact vectoriel
                                            vectorial_impact_value = sum(p['predicted_pips'] * p['direction'] for p in predictions)
                                            first_event_time = min(pd.to_datetime(p['event']['ts_utc']) for p in predictions)
                                            
                                            events_for_generator = [{
                                                'event_time': first_event_time,
                                                'predicted_pips': abs(vectorial_impact_value),
                                            ...
                                            #                                                     'latency_median': pred['latency_median'],
                                            #                                                     'ttr_median': pred['ttr_median'],
                                            #                                                     'label': f"{pred['event']['family']} ({pred['event']['country']})",
                                            #                                                     'is_phase': False
                                            #                                                 })
                                            #                                             
                                            st.warning("⚠️ Graphique basé sur predictions individuelles (mode séquentiel non actif)")

                                        # ❌ CORRECTION : Boucle qui ÉCRASAIT events_for_generator (COMMENTÉE)
                                        # Cette boucle ajoutait TOUS les événements individuels,
                                        # annulant le calcul vectoriel fait juste avant
                                        #
#                                        for pred in predictions:
#                                            events_for_generator.append({
#                                                'event_time': pd.to_datetime(pred['event']['ts_utc']),
...
#                                            })
                                        ...
                                        if price_df is not None and len(price_df) > 0:
                                            ...
                                            with col_s1:
                                                st.metric("Prix Maximum", f"{max_price:.5f}", 
                                                         delta=f"{(max_price - start_price_input) * 10000:+.1f} pips")
                                            
                                            with col_s2:
                                                st.metric("Prix Minimum", f"{min_price:.5f}", 
                                                         delta=f"{(min_price - start_price_input) * 10000:+.1f} pips")
                                            
                                            with col_s3:
                                                st.metric(f"Prix Final (T+{duration_minutes} min)", f"{end_price:.5f}", 
                                                         delta=f"{(end_price - start_price_input) * 10000:+.1f} pips")
                                            
                                            with col_s4:
                                                amplitude = (max_price - min_price) * 10000
                                                st.metric("Amplitude Totale", f"{amplitude:.1f} pips", 
                                                         help="Différence entre max et min")
                                            ...
                                            with st.expander("📖 Guide d'Utilisation"):
                                                ...
                                                ### 📏 Niveaux Fibonacci
                                                - Zones de support/résistance potentielles
                                                
                                                ### 📍 Marqueurs d'Événements
                                                - Lignes verticales noires au moment des événements
                                                
                                                ### ⚠️ Avertissement
                                                Simulation prédictive basée sur des modèles statistiques.
                                                """)
