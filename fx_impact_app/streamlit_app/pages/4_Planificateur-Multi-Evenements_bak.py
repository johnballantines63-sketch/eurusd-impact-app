...
if st.sidebar.button("🔍 Charger Événements", type="primary", use_container_width=True):
    with st.spinner("Chargement des événements..."):
        ...
        # Dédupliquer les événements par timestamp et famille
        if len(events) > 0:
    ...
    if len(st.session_state.selected_events) > 0:
        ...
        for idx in sorted(st.session_state.selected_events):
            ...
            with st.expander(f"📊 {event['family']} - {pd.to_datetime(event['ts_utc']).strftime('%H:%M')} ({event['country']})", expanded=True):
                ...
                if surprise != 0:  # ✅ Prédiction calculée même sans famille assignée
                            ...
                            with st.expander("⚙️ Paramètres du Graphique", expanded=True):
                                ...
                                with col_g1:
                                    start_price_input = st.number_input(
                                        ...
                                        key="minute_chart_price_144506_8655"
                                    )
                                ...
                                with col_g2:
                                    spread_pips = st.number_input(
                                        ...
                                        key="minute_chart_spread_144506_8655"
                                    )
                                ...
                                with col_g3:
                                    duration_minutes = st.number_input(
                                        ...
                                        key="minute_chart_duration_144506_8655"
                                    )
                                ...
                                with col_g4:
                                    volatility_factor = st.slider(
                                        ...
                                        key="minute_chart_volatility_144506_8655"
                                    )
                                ...
                                with st.expander("⚙️ Options Avancées"):
                                    show_bid_ask = st.checkbox(
                                        ...
                                        key="minute_chart_bid_ask_144506_8655"
                                    )
                                    ...
                                    show_fibonacci = st.checkbox(
                                        ...
                                        key="minute_chart_fibonacci_144506_8655"
                                    )
                            
                            # Bouton génération
                            if st.button("🎨 Générer Graphique de Prédiction", type="primary", use_container_width=True, key="minute_chart_generate_144506_8655"):
                                with st.spinner("🎨 Génération du graphique minute par minute..."):
                                    try:
                                        # Préparer données événements pour le générateur
                                        events_for_generator = []
                                        for pred in predictions:
                                            events_for_generator.append({
                                                'time': pd.to_datetime(pred['event']['ts_utc']),
                                                'impact_pips': pred['predicted_pips'] * pred['direction'],
                                                'latency_minutes': pred['latency_median'],
                                                'ttr_minutes': pred['ttr_median'],
                                        ...
                                        if price_df is not None and len(price_df) > 0:
                                            ...
                                            if show_fibonacci:
                                                ...
                                                dominant_movement = max_movement if abs(max_movement) > abs(min_movement) else min_movement
                                                
                                                fib_levels = calculate_fibonacci_price_levels(
                                                    start_price=start_price_input,
                                                    movement_pips=dominant_movement
                                                )
                                            ...
                                            st.plotly_chart(fig, use_container_width=True, key="minute_prediction_chart_144506_8655")
                                            ...
                                            with col_s3:
                                                st.metric(
                                                    f"Prix Final (T+{duration_minutes} min)",
                                                    f"{end_price:.5f}",
                                            ...
                                            # Guide d'utilisation
                                            with st.expander("📖 Guide d'Utilisation du Graphique"):
                                                st.markdown("""
                                                ### 🕯️ Chandeliers (Candlesticks)
                                                - **Vert** : Prix de clôture > prix d'ouverture (hausse)
                                                - **Rouge** : Prix de clôture < prix d'ouverture (baisse)
                                                - **Mèche haute** : Prix maximum de la période
                                                - **Mèche basse** : Prix minimum de la période
                                                
                                                ### 📏 Niveaux Fibonacci
                                                - **23.6%**, **38.2%**, **50%**, **61.8%**, **78.6%** : Niveaux de retracement
                                                - Zones de support/résistance potentielles
                                                - Plus le niveau est élevé, plus le retracement est profond
                                                
                                                ### 📍 Marqueurs d'Événements
                                                - **Lignes verticales noires** : Moment précis des événements
                                                - Labels indiquant le nom de l'événement
                                                
                                                ### 🎨 Zones Colorées
                                                - **Gris** : Phase de latence (avant réaction du marché)
                                                - **Vert/Rouge** : Phase d'impact (mouvement principal)
                                                - **Orange** : Phase de consolidation (après TTR)
                                                
                                                ### 💹 Lignes Bid/Ask
                                                - **Bleue pointillée** : Prix Bid (vente)
                                                - **Rouge pointillée** : Prix Ask (achat)
                                                - L'écart représente le spread
                                                
                                                ### ⚠️ Avertissement
                                                Cette simulation est **prédictive** et basée sur des modèles statistiques. 
                                                Elle ne garantit pas les mouvements réels du marché. Utilisez-la comme 
                                                outil d'analyse et non comme signal de trading absolu.
                                                """)
                                            
                                            if spread_pips > 0:
                                                st.info(f"📊 **Spread appliqué** : {spread_pips} pips (Bid-Ask)")
                                        
                                        else:
                                            st.error("❌ Erreur lors de la génération de la courbe de prix")
                                    ...
                                    with st.spinner("🔍 Analyse des événements historiques..."):
                                        ...
                                        for family in families_to_test:
                                            ...
                                            with st.expander(f"📋 Détail des {len(results)} événements testés", expanded=False):
                                                ...
                                                df_display['Erreur'] = df_display['Erreur'].apply(lambda x: f"{x:.1f}")
