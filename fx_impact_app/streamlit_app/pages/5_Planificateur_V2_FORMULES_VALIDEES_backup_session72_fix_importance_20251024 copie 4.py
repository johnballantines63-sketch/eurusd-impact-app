...
Version 2.6 - Session 99 (Amplification Optimale 1.0)
Utilise EXACTEMENT la méthode validée Session 55 + détection automatique type de mouvement
Correction Session 100 : Pullback utilise l'impact calculé (au lieu de valeur fixe 37.4)
Optimisation Session 99 : Amplification = 1.0 (validé sur 30 dates CPI, MAE réduit de 10.7%)
...
st.markdown("**Version 2.6** - Méthode Session 55 + Amplification 1.0 (Session 99) + Correction Bug Pullback (Session 100)")
...
def calculate_predictions(cpi_events: pd.DataFrame) -> dict:
    ...
    # SESSION 99 : Amplification optimale 1.0 (validée sur 30 dates CPI)
    # Amélioration 10.7% vs baseline 2.5 (MAE 12.24 vs 13.70 pips)
    impact = calculate_impact_d(
        empirical_score=adjusted_score,
        num_events=len(cpi_events),
        amplification=1.0  # Optimisé Session 99 (au lieu de 2.5)
    )
    ...
    # SESSION 100 : Correction bug - utiliser impact calculé au lieu de valeur fixe 37.4
    pullback = calculate_pullback_v2(impact, 10, 15)
...
if st.button("🎯 Calculer Prédictions", type="primary"):
    ...
    with st.spinner("Récupération des événements HIGH impact..."):
        ...
        st.write(f"🐛 DEBUG - Événements HIGH trouvés: {len(high_events)}")
    
    if high_events.empty:
        st.warning(f"❌ Aucun événement HIGH impact trouvé pour le {target_date.strftime('%d/%m/%Y')}")
        st.info("💡 Session 68-71 : Traite TOUS événements HIGH (score > 40) : CPI, NFP, Retail Sales, etc.")
        st.stop()
...
st.markdown("---")
st.markdown("""
**Planificateur V2** - Version 2.6 (Session 99 - Amplification Optimale 1.0)  
...
🔧 **CORRIGÉ (Session 100)** : Pullback utilise l'impact calculé (au lieu de 37.4 fixe)  
🎯 **OPTIMISÉ (Session 99)** : Amplification = 1.0 (validée sur 30 dates CPI, amélioration MAE 10.7%)  
