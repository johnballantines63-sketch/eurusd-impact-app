...
- AMÉLIORATION : Amplification plafonnée à ×2.5 (au lieu de ×10+)
- AMÉLIORATION : Surprises aberrantes plafonnées à 30%
- NOUVEAU : Filtrage événements faible importance (score < 40)
- RÉSULTAT : MAE réduit de 384% à 117% (-69% !)
...
print("✅ SESSION 46 : FICHIER MODIFIÉ AVEC DEBUG PULLBACK")  # ← NOUVEAU
...
def calculate_amplification_factor(surprise_pct: float, empirical_score: float = None) -> float:
    """
...
def calculate_pullback(
    ...
    AMÉLIORATION Session 53 (23 oct 2025):
    - Formule logarithmique au lieu de linéaire
    - Meilleure précision: MAE 0.2 pips (vs 12.1 pips v1)
    - Comportement réaliste: décroissance naturelle
    - Plafond adaptatif: 75% atteint progressivement (~11 min)
    
    RATIONALE:
    Le pullback suit une courbe logarithmique car:
    1. Forte correction initiale (panic selling/buying)
    2. Ralentissement progressif (absorption)
    3. Saturation naturelle (nouvel équilibre)
    ...
    Examples:
        >>> calculate_pullback(37.4, 10, 15)
        26.9  # 72% du mouvement après 10 min
        
        >>> calculate_pullback(50.0, 5, 20)
        27.0  # 54% du mouvement après 5 min
        
        >>> calculate_pullback(37.4, 10, 35)
...
def sequence_multi_event_timeline(
    ...
    # 🆕 SESSION 44 : Tracker pullback pour chaque phase
    ...
    for phase_idx, phase in enumerate(grouped_phases_with_impacts):
        ...
        # ✅ SESSION 46 : Lecture robuste avec fallback sur impact_combined
        impact = phase.get('impact_combined', phase.get('impact', 0))  # Priorité à impact_combined si disponible
        
        if debug:
            print(f"\n📍 Phase {phase_idx + 1}: {phase['event_name']}")
            print(f"   Minute: {phase_start_minute}, Impact: {impact:.1f} pips, Durée: {phase_duration} min")
            print(f"   🔍 DEBUG S46: phase['impact'] = {phase.get('impact', 'N/A')}")
            print(f"   🔍 DEBUG S46: phase['impact_combined'] = {phase.get('impact_combined', 'N/A')}")
            print(f"   🔍 DEBUG S46: prev_phase_impact (pour pullback) = {prev_phase_impact:.1f}")
        
        # 🔄 Gestion du pullback entre phases rapprochées
        pullback_pips = 0.0
        print(f"\n🔍 S46 DEBUG PULLBACK - Phase {phase_idx + 1}")  # ← PRINT INCONDITIONNEL
        print(f"   phase_idx = {phase_idx}, prev_phase_peak_time = {prev_phase_peak_time}")  # ← PRINT INCONDITIONNEL
        if phase_idx > 0 and prev_phase_peak_time is not None:
            ...
            print(f"   minutes_to_next_phase = {minutes_to_next_phase:.1f} (seuil = 30)")  # ← PRINT INCONDITIONNEL
            
            if minutes_to_next_phase < 30:
                # Calculer le pullback depuis le pic de la phase précédente
                minutes_since_peak = minutes_to_next_phase
                print(f"   ⚠️ prev_phase_impact AVANT calculate_pullback = {prev_phase_impact:.1f} pips")  # ← PRINT INCONDITIONNEL
                ...
                print(f"   ✅ pullback_pips CALCULÉ = {pullback_pips:.1f} pips")  # ← PRINT INCONDITIONNEL
                
                # 🆕 SESSION 44 : Sauvegarder le pullback
                # ✅ CORRECTION S45 : Associer pullback à phase PRÉCÉDENTE
    ...
    for idx, phase in enumerate(grouped_phases_with_impacts):
        ...
        # 🆕 SESSION 44 : Récupérer le pullback réel depuis phase_pullbacks
        enriched_phase['pullback_pips'] = phase_pullbacks.get(idx, 0.0)
