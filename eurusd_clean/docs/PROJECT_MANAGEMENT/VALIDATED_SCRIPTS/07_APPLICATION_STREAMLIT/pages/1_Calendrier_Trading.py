...
from core.event_families import FAMILY_PATTERNS, FAMILY_IMPORTANCE, FAMILY_DESCRIPTIONS
...
@st.cache_resource
def init_engines():
    return ForecastEngine(config.DB_PATH), ScoringEngine()
...
def load_precomputed_stats_from_db():
    """Charge événements depuis DB (version simplifiée fonctionnelle)"""
    try:
        conn = duckdb.connect(str(config.DB_PATH), read_only=True)
        
        # Simple: charger events avec leurs données de base
        query = """
            SELECT 
        ...
        for row in results:
            ...
            stats_dict[key] = {
                'event_title': row[2],
                'importance_n': row[3] if row[3] else 2,
                'empirical_score': 50.0,  # Valeur par défaut
            }
        return stats_dict
    except Exception as e:
...
def get_future_events(date_from, date_to, countries, min_importance=1):
    ...
    # ✅ Query avec event_title ajouté
    query = f"""
    SELECT 
        e.ts_utc, e.event_key, e.country, e.importance_n,
        e.actual, e.forecast, e.previous, e.event_title
    FROM events e
    ...
    if len(df) == 0:
        ...
        df = pd.DataFrame(columns=[
            ...
            '', 'family', 'impact_calendar', 'impact_empirical'
        ])
    else:
        ...
        df['empirical_score'] = df.apply(
            lambda row: get_stats_with_mapping(row['event_key'], row['country']).get('empirical_score', 'Unknown'), 
            axis=1
        )
        df[''] = df.apply(
            lambda row: get_stats_with_mapping(row['event_key'], row['country']).get('', 0), 
            axis=1
        )
        df[''] = df.apply(
            lambda row: get_stats_with_mapping(row['event_key'], row['country']).get('', 0), 
        ...
        df['impact_empirical'] = df['empirical_score'].fillna('Unknown')
    ...
    with st.spinner("🔄 Récupération des événements futurs..."):
        ...
        for _, event in future_events.iterrows():
            ...
            if has_empirical:
                ...
                enriched_events.append({
                    ...
                    'event_title': event.get('event_title', event['event_key']),
                    'family': family,
                    'country': event['country'],
                    'importance': event['importance_n'],
                    'empirical_score': event.get('empirical_score', 'Unknown'),
                ...
                enriched_events.append({
                    ...
                    'event': event['event_key'],
                    'event_original_key': event['event_key'],
                    'event_title': event.get('event_title', event['event_key']),  # ✅ Pour lookup stats
                    'family': family or 'Autre',
                    'country': event['country'],
                    'importance': event['importance_n'],
                    'empirical_score': event.get('empirical_score', 'Unknown'),  # ✅ Ajouter
                    'score': 0,
                    'grade': 'N/A',
                    'tradability': 'N/A',
                    'impact_p80': 0,
        ...
        for date_str in sorted(events_by_date.keys(), key=lambda x: datetime.strptime(x, '%d/%m/%Y')):
            ...
            for event in sorted(events_today, key=lambda x: x['time']):
                ...
                # Utiliser empirical_score si disponible, sinon calendrier
                if use_empirical:
                    # Mode Empirique : priorité aux données vérifiées
                    if event.get('empirical_score') and event['empirical_score'] != 'Unknown':
                        impact_display = event['empirical_score']
                ...
                with st.expander(f"{badge} **{event['time']}** | {imp_stars} | **{event['family']}** - {event.get('event_title', event.get('event_key', 'N/A'))} ({event['country']}) | Score: {event['score']:.0f}/100"):
                    ...
                    if use_empirical and event.get('empirical_score') and event['empirical_score'] != 'Unknown':
                        st.divider()
                        ...
                        with col1:
                            ...
                            precomputed = st.session_state.get('precomputed_stats', {})
                            event_key = event.get('event_original_key', event.get('event_title', event.get('event_key', 'N/A')))
                            country = event['country']
                            ...
                            st.metric(
                                "🎯 Impact Vérifié", 
                                event.get('empirical_score', 'N/A'),
                                help="Impact réel observé sur historique"
                            )
                            st.metric(
                                "📈 Mouvement Moyen", 
                                f"{stats.get('') or 0:.1f} pips",
