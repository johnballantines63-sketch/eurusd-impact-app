"""
Script de validation formule S115 - VERSION PROFESSIONNELLE
Session 118 - Récupération latency_median depuis DB

APPROCHE SCIENTIFIQUE:
- Récupérer latency_median depuis event_families (statistiques empiriques)
- Valeur par défaut (2.0) uniquement si event non trouvé dans DB
"""
...
print("=" * 80)
print("🚀 VALIDATION FORMULE S115 - SESSION 118 (PROFESSIONNEL)")
print("=" * 80)
...
# ============================================================================
# ÉTAPE 2: CONNEXION DATABASE & CHARGEMENT EVENT_FAMILIES
# ============================================================================
print("\n" + "=" * 80)
print("ÉTAPE 2: CONNEXION DATABASE & CHARGEMENT LATENCY_MEDIAN")
print("=" * 80)
...
# Charger event_families pour récupérer latency_median
try:
    ...
    # Créer dictionnaire event_key → latency_median
    latency_map = dict(zip(
        event_families_df['event_key'].str.lower(), 
        event_families_df['latency_median']
    ))
    
    print(f"✅ Chargé {len(latency_map)} event_families avec latency_median")
    
except Exception as e:
    print(f"⚠️  Erreur chargement event_families: {e}")
    print(f"⚠️  Utilisation valeur par défaut pour tous les events")
    latency_map = {}

# ============================================================================
# FONCTION: ENRICHIR EVENTS AVEC LATENCY_MEDIAN
# ============================================================================

def enrich_events_with_latency(events_list, latency_map):
    """
    Enrichir liste d'events avec latency_median depuis DB
    
    Args:
        events_list: Liste dicts events
        latency_map: Dict event_key → latency_median
    
    Returns:
        DataFrame avec colonne latency_median
    """
    # Créer DataFrame
    df = pd.DataFrame(events_list)
    
    # Ajouter latency_median depuis map
    def get_latency(event_key):
        if pd.isna(event_key):
            return 2.0  # Défaut si event_key manquant
        
        key_lower = str(event_key).lower()
        return latency_map.get(key_lower, 2.0)  # 2.0 si pas trouvé
    
    df['latency_median'] = df['event_key'].apply(get_latency)
...
for i, case in enumerate(validable_cases, 1):
    ...
    try:
        ...
        # Analyser distribution temporelle des events
        all_events = case.get('events', [])
        if not all_events:
            print(f"   ⚠️ Aucun event disponible, skip")
            continue
        
        event_times = [pd.to_datetime(e['datetime']) for e in all_events]
        time_span = (max(event_times) - min(event_times)).total_seconds() / 60
        
        print(f"   📊 {len(all_events)} events, span: {time_span:.1f} min")
        
        # STRATÉGIE DE SÉPARATION DES EVENTS
        if time_span < 5:
            # Tous les events simultanés → 1 cluster créant 2 vagues
            print(f"   📌 Pattern: 1 cluster → 2 vagues")
            wave1_events = all_events
            wave2_events = all_events
            
        else:
            # Events séparés → Tenter 2 clusters
            print(f"   📌 Pattern: 2 clusters possibles")
            separation_time = peak1_time + timedelta(minutes=timing_delta_minutes/2)
            
            wave1_events = [e for e in all_events 
                           if pd.to_datetime(e['datetime']) <= separation_time]
            wave2_events = [e for e in all_events 
                           if pd.to_datetime(e['datetime']) > separation_time]
            
            if not wave1_events or not wave2_events:
                print(f"   ⚠️ Séparation impossible, 1 cluster")
                wave1_events = all_events
                wave2_events = all_events
            else:
                print(f"   ✅ Séparé: W1={len(wave1_events)}, W2={len(wave2_events)}")
        
        # ENRICHIR AVEC LATENCY_MEDIAN depuis DB
        wave1_df = enrich_events_with_latency(wave1_events, latency_map)
        wave2_df = enrich_events_with_latency(wave2_events, latency_map)
        
        # Stats latency
        latency_w1 = wave1_df['latency_median'].median()
        latency_w2 = wave2_df['latency_median'].median()
        print(f"   📐 Latency: W1={latency_w1:.1f} min, W2={latency_w2:.1f} min")
        ...
        print(f"   W1:       {wave1_result['impact_pips']:.2f} pips")
        print(f"   W2:       {wave2_result['impact_pips']:.2f} pips")
...
results_json = {
    'metadata': {
        ...
        'latency_source': 'event_families (DB)'
    },
    'statistics': {
