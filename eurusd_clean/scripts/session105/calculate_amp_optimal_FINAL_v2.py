#!/usr/bin/env python3
"""
SESSION 105 - CALCUL amp_optimal FINAL
=======================================
CORRECTION : Filtre temporel 14:00-15:00+02:00 (heure Bern)
"""
...
print("="*80)
print("SESSION 105 - CALCUL amp_optimal FINAL")
print("="*80)
print()
...
for idx, date_str in enumerate(sorted(measurements.keys()), 1):
    ...
    # Charger événements US 14:30 Bern (stockés 14:30+02:00 dans DB)
    query = f"""
    SELECT 
        ef.family,
        COALESCE(
    ...
    WHERE DATE(e.ts_utc) = '{date_str}'
        ...
        AND e.ts_utc >= '{date_str} 14:00:00+02:00'::TIMESTAMP
        AND e.ts_utc < '{date_str} 15:00:00+02:00'::TIMESTAMP
    ORDER BY e.ts_utc
    """
    ...
    if len(events_df) == 0:
        print(f"   ⚠️ Pas d'événements 14:30, essai sans filtre heure...")
        
        # Fallback : sans filtre temporel
        query = f"""
        SELECT 
            ef.family,
            COALESCE(
...
if len(ref_row) > 0:
    ...
    if abs(amp_ref - 2.5) < 0.5:
        ...
        print(f"⚠️ Date référence 11.09 : amp = {amp_ref:.3f} (écart vs 2.5)")
    print()
...
print()
print("="*80)
print("✅ SESSION 105 - ÉTAPE 3 TERMINÉE")
print("="*80)
