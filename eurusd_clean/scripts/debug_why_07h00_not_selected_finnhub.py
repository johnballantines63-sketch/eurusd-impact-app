"""
Debug : Pourquoi le pic à 07:00 H1 Finnhub n'est pas sélectionné ?
"""
import sys
...
# Pic attendu à 07:00 le 09.09 (high=1.17801)
expected_pic = tz.localize(datetime(2025, 9, 9, 7, 0))
expected_pic_idx = prices_series.index.get_indexer([expected_pic], method='nearest')[0]

# Segment détecté à 03:00 le 09.09
detected_start = tz.localize(datetime(2025, 9, 9, 3, 0))
detected_start_idx = prices_series.index.get_indexer([detected_start], method='nearest')[0]

print('='*80)
print('COMPARAISON DES SEGMENTS H1 FINNHUB')
print('='*80)
print(f'Événement : {event_time.strftime("%Y-%m-%d %H:%M")} (index {event_idx})')
print()

# Segment 1 : 07:00 → 14:30
...
length1_hours = length1
distance1_hours = distance1
...
if score1 > score2:
    print(f'✅ Segment 1 (07:00) devrait être sélectionné (score {score1:.4f} > {score2:.4f})')
    print(f'   Mais il ne l\'est pas ! Pourquoi ?')
    print(f'   → Vérifier si 07:00 est dans la liste des candidats testés')
else:
    print(f'⚠️  Segment 2 (03:00) est sélectionné car score {score2:.4f} > {score1:.4f}')
