#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# (full content will be inserted below in a second write)
...
import duckdb, numpy as np, pandas as pd, pytz, matplotlib.pyplot as plt
...
def fetch_prices_bern(conn, start_ts, end_ts):
    ...
    df = conn.execute(q, [start_ts.to_pydatetime(), end_ts.to_pydatetime()]).df()
...
def plot_result(df, res, out_path, title_prefix='EUR/USD – Double Wave (math)'):
    ...
    plt.axvline(pd.to_datetime(res.baseline_time), linestyle='--', alpha=0.6)
    mark(res.baseline_time, res.baseline_price, 'Baseline')
    mark(res.peak1_time, res.peak1_price, 'Peak1')
    mark(res.pullback1_time, res.pullback1_price, 'Pullback1')
    mark(res.peak2_time, res.peak2_price, 'Wave2')
    mark(res.pullback2_time, res.pullback2_price, 'Pullback2')
    ttl = f"{title_prefix} | {res.direction} | W1 {res.wave1_amp_pips}p / W2 {res.wave2_amp_pips}p"
...
def main():
    ...
    if res is None:
        ...
        print("No double wave detected."); return
    Path(OUT_JSON).write_text(json.dumps(asdict(res), indent=2, ensure_ascii=False), encoding="utf-8")
    print("Saved JSON →", OUT_JSON)
    out_dir = Path(PLOT_DIR); out_dir.mkdir(exist_ok=True, parents=True)
    png_path = out_dir / f"double_wave_{label}.png"
    plot_result(df, res, png_path); print("Saved plot →", png_path)
