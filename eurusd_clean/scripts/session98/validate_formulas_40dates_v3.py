"""
Script de validation formules S51-55 sur 40 dates CPI - VERSION 3 FINALE
...
print("=" * 100)
print("🔍 VALIDATION FORMULES S51-55 SUR 40 DATES CPI - VERSION 3 FINALE")
print("=" * 100)
print(f"\n📊 Database: {DB_PATH}")
...
def load_events_for_date(conn, date_str: str, time_str: str):
    ...
    """
    # Construire timestamp complet pour comparaison
    # Format: '2025-03-12 13:30:00+02:00'
    timestamp_str = f"{date_str} {time_str}+02:00"
    
    # Query avec JOIN pour récupérer empirical_score depuis event_families
    # Comparaison directe avec timestamp complet
    query = """
    SELECT 
        e.event_title,
        e.event_key,
    ...
    df_events = conn.execute(query, [timestamp_str]).df()
...
for idx, row in df_real.iterrows():
    ...
    try:
        ...
        print(f"📊 Événements: {' | '.join(event_titles)}")
