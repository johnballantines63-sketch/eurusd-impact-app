"""
Script de validation formules S51-55 sur 40 dates CPI
...
print("=" * 100)
print("🔍 VALIDATION FORMULES S51-55 SUR 40 DATES CPI")
print("=" * 100)
print(f"\n📊 Database: {DB_PATH}")
...
def load_events_for_date(conn, date_str: str, time_str: str):
    """
    Charge tous les événements pour une date/heure donnée
    ...
    Returns:
        DataFrame avec événements (name, empirical_score, actual, estimate, forecast, previous)
    """
    # Parser datetime
...
for idx, row in df_real.iterrows():
    ...
    try:
        ...
        print(f"📊 Événements: {' | '.join(df_events['name'].tolist())}")
        
        # Calculer score max et moyen
        score_max = df_events['empirical_score'].max()
        score_avg = df_events['empirical_score'].mean()
        ...
        results.append({
            ...
            'score_max': score_max,
            'score_avg': score_avg,
            'events': ' | '.join(df_events['name'].tolist()),
