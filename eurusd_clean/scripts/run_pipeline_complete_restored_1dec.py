...
VERSION RESTAURÉE (1er décembre 2025)
Cette version restaure le pipeline tel qu'il était avant les modifications du 3 décembre 2025.

Modifications retirées :
- Seuil adaptatif noyau dur (retour au seuil fixe)
- Support calculé uniquement dans clusters du même type (pas tous clusters)
- Méthode Session 88 (retour au calcul individuel)
- Stratégie hybride conditionnelle (retour à stratégie identique pour tous)
- Timings parfaits Session 64 (retour à détection depuis prix réels)
...
class PipelineExecutor:
    ...
    def _calculate_historical_support(
        ...
        # ✅ VERSION RESTAURÉE : Support calculé uniquement dans clusters du même type
        support_scores = {}
        for event_id in event_ids:
            # Pour tous les événements : utiliser support dans clusters du type uniquement
        ...
        if core_type != 'GENERIC':
            ...
            for event_id, support in support_scores.items():
                ...
                if event_found is not None:
                    ...
                    # ✅ VERSION RESTAURÉE : Seuil fixe uniquement (pas de seuil adaptatif)
    ...
    def etape8_appliquer_cluster_cible(
        ...
        # 8.1 : Calcul de l'Impact de Base (Méthode Standard - Version Restaurée)
        # ✅ VERSION RESTAURÉE : Calcul individuel pour chaque événement (comme Étape 6)
        # Méthode standard : somme des impacts individuels avec correction vectorielle
        ...
        # Correction vectorielle après (pour multi-événements)
        if num_events >= 2:
        ...
        self._log(f"   📊 Impact de base (méthode standard): {impact_base:.2f} pips", "INFO")
        ...
        # 8.6 : Détection Pattern de Prix (Version Restaurée)
        # ✅ VERSION RESTAURÉE : Détection pattern depuis prix réels uniquement (pas de timings prédits)
        try:
            import sys
            ...
            # Détecter pattern depuis prix réels
            ...
            if pattern_result:
                ...
                self._log(f"   ℹ️ Aucun pattern détecté", "INFO")
        except Exception as e:
        ...
        # ✅ VERSION RESTAURÉE : Détection pattern depuis prix réels uniquement
        # Pas de timings prédits Session 64, pas de validation pattern réel vs critères événements
        # Le code ci-dessus détecte directement le pattern depuis les prix réels
        ...
        # ✅ VERSION RESTAURÉE : Stratégie identique pour tous les patterns
        # Option C (révisée) : Stratégie hybride standard pour tous
