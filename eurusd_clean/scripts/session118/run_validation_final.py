"""
Script de validation formule S115 - VERSION FINALE
Session 118 - Correction colonne latency_median manquante

Correction: Ajouter latency_median par défaut (2.0 min) aux events
"""
...
print("=" * 80)
print("🚀 VALIDATION FORMULE S115 - SESSION 118 (FINAL)")
print("=" * 80)
    ...
    try:
        ...
        if time_span < 5:
            ...
            if not wave1_events or not wave2_events:
                ...
                print(f"   ✅ Séparé: W1={len(wave1_events)} events, W2={len(wave2_events)} events")
        
        # CRITIQUE: Ajouter latency_median manquant (valeur par défaut)
        if 'latency_median' not in wave1_df.columns:
            wave1_df['latency_median'] = 2.0  # Défaut: 2 min (observation empirique)
        if 'latency_median' not in wave2_df.columns:
            wave2_df['latency_median'] = 2.0
