"""
Script de validation formule S115 - VERSION CORRIGÉE
Session 118

Correction: calculate_cluster_impact() ne prend pas de paramètre conn
Adaptation: Gestion des cas où tous les events sont au même moment
"""
...
print("=" * 80)
print("🚀 VALIDATION FORMULE S115 - SESSION 118 (CORRECTED)")
print("=" * 80)
...
for i, case in enumerate(validable_cases, 1):
    ...
    try:
        ...
        print(f"   📊 {len(all_events)} events, span temporel: {time_span:.1f} min")
        
        # STRATÉGIE DE SÉPARATION DES EVENTS
        if time_span < 5:
            # Tous les events sont simultanés (< 5 min d'écart)
            # → UN SEUL cluster qui crée 2 vagues
            print(f"   📌 Pattern: 1 cluster → 2 vagues (events simultanés)")
            
            # Utiliser TOUS les events pour Wave1
            # Wave2 = même cluster mais impact depuis creux
            wave1_df = pd.DataFrame(all_events)
            wave2_df = pd.DataFrame(all_events)  # Même cluster
            
        else:
            # Events séparés dans le temps
            # → Essayer de séparer en 2 clusters
            print(f"   📌 Pattern: 2 clusters distincts possibles")
            
            # Point de séparation = milieu temporel entre peak1 et wave2
            ...
            # Si un cluster est vide, tout mettre dans Wave1
            if not wave1_events or not wave2_events:
                print(f"   ⚠️ Séparation impossible, traiter comme 1 cluster")
                wave1_df = pd.DataFrame(all_events)
                wave2_df = pd.DataFrame(all_events)
            else:
                wave1_df = pd.DataFrame(wave1_events)
                wave2_df = pd.DataFrame(wave2_events)
                print(f"   ✅ Séparation: Wave1 {len(wave1_events)} events, Wave2 {len(wave2_events)} events")
        
        # Calculer impacts clusters (SANS paramètre conn)
        ...
        print(f"   Wave1:    {wave1_result['impact_pips']:.2f} pips")
        print(f"   Wave2:    {wave2_result['impact_pips']:.2f} pips")
...
if len(outliers) > 0:
    ...
    for _, row in outliers.iterrows():
        ...
        print(f"     Momentum {row['momentum_factor']:.3f}, Events span {row['events_span_min']:.1f} min")
else:
...
results_json = {
    'metadata': {
        ...
        'note': 'Version corrigée - gestion 1 cluster → 2 vagues'
    },
    'statistics': {
