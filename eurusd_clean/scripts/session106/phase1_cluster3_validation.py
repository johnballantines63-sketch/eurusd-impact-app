#!/usr/bin/env python3
"""
SESSION 106 - PHASE 1 : VALIDATION CLUSTER #3 (CPI)
====================================================

Teste la méthode Planificateur V2.4 sur les 6 dates du Cluster #3

Dates Cluster #3 (CPI - 11 événements) :
- 2025-09-11 ✅ (référence validée : 56.26 vs 56.8 pips)
- 2025-08-12
- 2025-07-15
- 2025-06-11
- 2025-05-13
...
2. Calculer impact prédit (amp=2.5 baseline)
3. Mesurer impact réel (méthode Session 92.5)
4. Calculer amp_optimal (scipy)
5. Analyser variabilité amp
...
print("="*80)
print("SESSION 106 - PHASE 1 : VALIDATION CLUSTER #3 (CPI)")
print("="*80)
print()
...
def measure_real_impact(date_str: str, event_time_bern: str) -> dict:
    """
    Mesure impact réel depuis prices_1m
    Méthode Session 92.5 (timestamps DB +02:00)
    """
    conn = duckdb.connect(str(db_path), read_only=True)
    
    # Convertir event_time_bern en timestamp DB
    # Exemple : "2025-09-11 14:30:00" Bern → "2025-09-11 12:30:00+02:00" DB
    event_dt = datetime.strptime(f"{date_str} {event_time_bern}", "%Y-%m-%d %H:%M:%S")
    event_dt_utc = event_dt - timedelta(hours=2)  # Bern → UTC
    
    # Fenêtre : 60 min après événement
    window_end = event_dt_utc + timedelta(minutes=60)
    ...
    df_prices = conn.execute(query, [
        event_dt_utc.strftime('%Y-%m-%d %H:%M:%S'),
...
for date_str in DATES_CLUSTER3:
    ...
    # Trouver heure événement (premier événement)
    event_time_utc = df_events.iloc[0]['ts_utc']
    event_time_bern = (pd.to_datetime(event_time_utc) + timedelta(hours=2)).strftime('%H:%M:%S')
    
    print(f"   Heure événement (Bern) : {event_time_bern}")
    print()
    ...
    # Mesurer impact réel
    real = measure_real_impact(date_str, event_time_bern)
    ...
    error_baseline_pct = (error_baseline / real['impact_pips']) * 100
    ...
    error_optimal_pct = (error_optimal / real['impact_pips']) * 100
...
for _, row in df_results.iterrows():
    print(f"{row['date']} : amp_optimal={row['amp_optimal']:.3f}, error_baseline={row['error_baseline']:.1f}p, error_optimal={row['error_optimal']:.1f}p")

print()

# Sauvegarder résultats
output_file = script_path.parent / "phase1_cluster3_results.csv"
