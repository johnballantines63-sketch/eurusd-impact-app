...
CORRECTION : Grouper par composition EXACTE du cluster, pas juste Type+Nombre

Logique correcte :
1. Identifier clusters qui se RÉPÈTENT (même event_keys)
2. Pour chaque cluster répété, mesurer impacts sur N occurrences
3. Analyser si surprise → impact ou si cluster → impact stable

Exemple :
- CPI mensuel (core_mom, core_yoy, cpi_mom, cpi_yoy, real_earnings)
- 60 occurrences sur 5 ans
- Surprise varie 10-80% mais impact stable ~50 pips ?
"""
...
SELECT 
    ...
    STRING_AGG(DISTINCT event_date::VARCHAR, ', ' ORDER BY event_date) as dates_list
