...
def extract_features_for_rf(
    ...
    """
    Extrait les features pour Random Forest à partir des données du cluster.
    ...
    - trend_direction_encoded : Direction de la tendance (1=UP, -1=DOWN, 0=UNKNOWN)
    - trend_amplitude_pips : Amplitude de la tendance (pips)
    
    Args:
        cluster_events: DataFrame des événements du cluster
        trend_r2: R² de la tendance (défaut: 0.0)
        trend_direction: Direction de la tendance (défaut: 'UNKNOWN')
        trend_amplitude_pips: Amplitude de la tendance (défaut: 0.0)
    ...
    if surprises:
        features['max_surprise_pct'] = max(surprises)
        features['mean_surprise_pct'] = np.mean(surprises)
    else:
        features['max_surprise_pct'] = 0.0
        features['mean_surprise_pct'] = 0.0
    
    # Nombre d'événements
    features['num_events'] = num_events if num_events > 0 else len(cluster_events)
    
    # Score empirique moyen
    empirical_scores = cluster_events['empirical_score'].dropna()
    if not empirical_scores.empty:
        features['mean_empirical_score'] = empirical_scores.mean()
    else:
        features['mean_empirical_score'] = 44.0  # Score par défaut
    
    # Features de tendance
    features['trend_r2'] = trend_r2
    features['trend_direction_encoded'] = 1.0 if trend_direction == 'UP' else (-1.0 if trend_direction == 'DOWN' else 0.0)
    features['trend_amplitude_pips'] = trend_amplitude_pips
...
def train_rf_from_identical_clusters(
    ...
    for _, row in valid_results.iterrows():
        ...
        trend_direction = row.get('direction', 'UNKNOWN') if 'direction' in results_df.columns else 'UNKNOWN'
