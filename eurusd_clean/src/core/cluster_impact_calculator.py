...
from .formulas_validated import (
    ...
    calculate_impact_d,
    calculate_impact_linear,  # ⭐ NOUVELLE FORMULE RECOMMANDÉE
    calculate_ttr_c,
...
def calculate_cluster_impact(
    ...
    amplification: float = 2.8,  # ⚠️ DÉPRÉCIÉ : Non utilisé avec calculate_impact_linear()
    use_linear_formula: bool = True  # ⭐ Utiliser nouvelle formule linéaire (recommandé)
) -> Dict:
    """
    ...
    def calculate_event_surprise(event_key, actual, estimate, previous=None):
        ...
        LOGIQUE Session 113 ORIGINALE - NE PAS MODIFIER:
    ...
    # ⭐ CORRECTION : Exclure événements avec surprise nulle (< 0.1%)
    # Ces événements n'influencent pas la direction et faussent la moyenne
    ...
    for _, event in cluster_events.iterrows():
        ...
        # ✅ Exclure surprises nulles ou très faibles (< 0.1%)
        # Ces événements n'ont pas d'influence sur la direction du cluster
    ...
    # ⭐ CORRECTION : Seuls événements avec surprise significative contribuent
    ...
    # 5. Calculer impact avec formule optimale
    num_events = len(cluster_events)
    
    if use_linear_formula:
        # ⭐ NOUVELLE FORMULE LINÉAIRE : Meilleure précision (MAE 13.98 vs 38.63 pips)
        # Calculer surprise moyenne et maximale pour formule linéaire
        surprise_avg = max_surprise  # Utiliser surprise nette comme moyenne
        surprise_max = max_surprise  # Utiliser surprise nette comme maximum
        ...
        formula_used_name = 'calculate_impact_linear (SESSION_VALIDATION_ACTUELLE)'
    else:
        # Formule D (rétrocompatibilité)
        impact_pips = calculate_impact_d(
            empirical_score=adjusted_score,
            num_events=num_events,
            amplification=amplification
        )
        formula_used_name = 'calculate_impact_d (Session 51) - DÉPRÉCIÉ'
    ...
    calculation_details = {
        ...
        'formula_used': f'{formula_used_name} + surprise nette vectorielle (Session 113)'
    }
